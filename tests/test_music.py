...
from fable_flow.music import generate_music_from_fn, produce
...
class TestMusicGeneration:
    """Test the actual music generation functionality with agents and output files."""

    @pytest.mark.asyncio
    async def test_music_agents_execute_without_error(
        self, test_data_dir: Path, test_output_dir: Path, ant_sugar_story_setup: dict[str, str]
    ) -> None:
        """Test that all music agents execute without error."""
        ...
        try:
            await generate_music_from_fn(story_fn=story_dir, destination=test_output_dir)
        except Exception as e:
            pytest.fail(f"Music agents failed to execute: {e}")

    @pytest.mark.asyncio
    async def test_music_agents_create_output_files(
        self, test_data_dir: Path, test_output_dir: Path, ant_sugar_story_setup: dict[str, str]
    ) -> None:
        """Test that music agents create non-empty output files."""
        story_dir: Path = test_data_dir / "ant_sugar"
        story_dir.mkdir()
    ...
    @pytest.mark.asyncio
    async def test_music_models_generate_output(
        self, test_data_dir: Path, test_output_dir: Path, ant_sugar_story_setup: dict[str, str]
    ) -> None:
        """Test that music models run correctly and generate output."""
        story_dir: Path = test_data_dir / "ant_sugar"
        story_dir.mkdir()
        ...
        for file_path in output_files:
            if file_path.is_file() and file_path.stat().st_size > 0:
                if file_path.suffix.lower() in [".mp3", ".wav", ".ogg", ".txt", ".json"]:
                    model_generated_content = True
                    break

        assert test_output_dir.exists(), "Output directory was not created"
        if not model_generated_content:
            pytest.skip("No music model output detected - may require API keys or model setup")
