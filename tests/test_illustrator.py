...
from fable_flow.illustrator import draw, main
...
class TestIllustrationGeneration:
    """Test the actual illustration generation functionality with agents and output files."""

    @pytest.mark.asyncio
    async def test_illustration_agents_execute_without_error(
        self, test_data_dir: Path, test_output_dir: Path, ant_sugar_story_setup: dict[str, str]
    ) -> None:
        """Test that all illustration agents execute without error."""
        ...
        try:
            await main(
                story_fn=story_dir,
                synopsis_fn=story_dir / "draft_synopsis.txt",
                output_dir=test_output_dir,
            )
        except Exception as e:
            pytest.fail(f"Illustration agents failed to execute: {e}")

    @pytest.mark.asyncio
    async def test_illustration_agents_create_output_files(
        self, test_data_dir: Path, test_output_dir: Path, ant_sugar_story_setup: dict[str, str]
    ) -> None:
        """Test that illustration agents create non-empty output files."""
        story_dir: Path = test_data_dir / "ant_sugar"
        story_dir.mkdir()
        ...
        assert len(output_files) > 0, "No output files created by illustration agents"
        assert non_empty_files > 0, "All output files are empty"

    def test_illustration_cli_command_wiring(
        ...
        with patch("asyncio.run") as mock_run:
            try:
                draw(None, story_dir, story_dir / "draft_synopsis.txt")
                mock_run.assert_called_once()
            except Exception as e:
                pytest.fail(f"CLI command wiring failed: {e}")

    @pytest.mark.asyncio
    async def test_illustration_models_generate_output(
        self, test_data_dir: Path, test_output_dir: Path, ant_sugar_story_setup: dict[str, str]
    ) -> None:
        """Test that illustration models run correctly and generate output."""
        story_dir: Path = test_data_dir / "ant_sugar"
        story_dir.mkdir()
        ...
        if not model_generated_content:
            pytest.skip(
                "No illustration model output detected - may require API keys or model setup"
            )
