...
from fable_flow.movie import generate_video_from_fn, produce
...
class TestMovieGeneration:
    """Test the actual movie/video generation functionality with agents and output files."""

    @pytest.mark.asyncio
    async def test_movie_agents_execute_without_error(
        self, test_data_dir: Path, test_output_dir: Path, ant_sugar_story_setup: dict[str, str]
    ) -> None:
        """Test that all movie agents execute without error."""
        story_dir: Path = test_data_dir / "ant_sugar"
        story_dir.mkdir()

        async with aiofiles.open(story_dir / "final_story.txt", "w") as f:
        ...
        try:
            await generate_video_from_fn(story_fn=story_dir, destination=test_output_dir)
        except Exception as e:
            pytest.fail(f"Movie agents failed to execute: {e}")

    @pytest.mark.asyncio
    async def test_movie_agents_create_output_files(
        self, test_data_dir: Path, test_output_dir: Path, ant_sugar_story_setup: dict[str, str]
    ) -> None:
        """Test that movie agents create non-empty output files."""
        story_dir: Path = test_data_dir / "ant_sugar"
        story_dir.mkdir()
        ...
        assert len(output_files) > 0, "No output files created by movie agents"
        assert non_empty_files > 0, "All output files are empty"

    def test_movie_cli_command_wiring(
        self, test_data_dir: Path, ant_sugar_story_setup: dict[str, str]
    ) -> None:
        """Test that the CLI command is wired correctly."""
        story_dir: Path = test_data_dir / "ant_sugar"
        story_dir.mkdir()
        destination_dir: Path = test_data_dir / "movie_output"
        destination_dir.mkdir()

        with open(story_dir / "final_story.txt", "w") as f:
        ...
        with patch("asyncio.run") as mock_run:
            try:
                produce(None, story_dir, destination_dir)
                mock_run.assert_called_once()
            except Exception as e:
                pytest.fail(f"CLI command wiring failed: {e}")

    @pytest.mark.asyncio
    async def test_movie_models_generate_output(
        self, test_data_dir: Path, test_output_dir: Path, ant_sugar_story_setup: dict[str, str]
    ) -> None:
        """Test that movie models run correctly and generate output."""
        story_dir: Path = test_data_dir / "ant_sugar"
        story_dir.mkdir()
        ...
        for file_path in output_files:
            if file_path.is_file() and file_path.stat().st_size > 0:
                if file_path.suffix.lower() in [".mp4", ".avi", ".mov", ".txt", ".png", ".jpg"]:
                    model_generated_content = True
                    break

        assert test_output_dir.exists(), "Output directory was not created"
        if not model_generated_content:
            pytest.skip("No movie model output detected - may require API keys or model setup")
